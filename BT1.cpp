...
void Shape::showEdge1Shape() // Ham in ra tat ca cac canh noi tam 1 hinh bat ki
    {
       int i;
...
class Graph
{
    public: 
        ...
        {
            ifstream file_input ("E:\\MaiAnh\\STUDY\\TEST\\.vscode\\file.txt");
            string getinput;
            char id;
            ...
            {   
                while (!file_input.eof()) // Duyet file tu dau toi cuoi file thi dung lai
                {
                    ...
                    cout << getinput << " cc" << endl;
                    char ip = getinput[0];
                    switch (ip) // Xet ki tu dau tien cua getinput
                    {
                        case 's': // Shape
                        {
                            getline(file_input, getinput, ',');
                            char id = getinput[0];
                            cout << getinput << " bb" << endl;
                            switch (id) // Xet ki tu thu 0 cua getinput
                            {
                                case 'c': // Cirlce
                                {
